...
Feature: Receive feedback from provider feature
  Seeker must be able to receive feedbacks from provider
  ...
  Scenario: Job is applied successfully
    ...
    And the job provider posts the following feedback to the application
    """
      {
    ...
    And the field "provider_feedbacks" returned must be '["Test Feedback."]'
